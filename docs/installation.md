...
> [!IMPORTANT]
> On the [GitHub Releases page](https://github.com/Kamionn/EasySwitchLua/releases), download the **named release asset** (`EasySwitchLua-vX.Y.Z.zip`), **not** "Source code (zip)". The source archive does not include the bundled dist — `dist/` is gitignored, the dist is only generated by the release CI workflow.
