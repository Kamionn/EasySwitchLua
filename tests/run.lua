...
test("pattern matching basics", function()
    local s = Switch("t")
        :when(P.string, function() return "s" end)
        :when(P.integer, function() return "i" end)
...
test("P.integer and P.float distinguish integers from floats", function()
    local s = Switch("t")
        :when(P.integer, function() return "i" end)
        :when(P.float, function() return "f" end)
        :default(function() return "?" end)
    assertEq(s:execute(2), "i")
    assertEq(s:execute(2.0), "f")
    assertEq(s:execute(2.5), "f")
    assertEq(s:execute("x"), "?")
end)
